...
public class TeamTypesWindow : INItializableWindow
{
    ...
    {
        QueueScriptingRefresh();
    }

    private void Map_ScriptingChanged(object sender, ScriptingChangedEventArgs e)
    {
        if (e.AffectsAny(typeof(TeamType), typeof(TaskForce), typeof(Script), typeof(Tag)))
            QueueScriptingRefresh();
    }

    private void QueueScriptingRefresh()
    {
        if (Visible)
    ...
    {
        var teamType = editedTeamType;
        if (teamType != null && !map.TeamTypes.Contains(teamType))
            teamType = null;
    ...
    private void ShowTeamTypeReferences()
    {
        ...
        {
            stringBuilder.AppendLine(
                string.Format(Translate(this, "GlobalTeamTypeOverridden", "- This TeamType overrides a global TeamType {0}. As such, it may be used by global AI Triggers."),
