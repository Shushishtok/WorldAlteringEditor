...
{
    /// <summary>
    /// Byte length of an RSA encrypted Blowfish key.
    /// </summary>
    ...
    public static byte[] DecryptBlowfishKey(byte[] encryptedBlowfishKey)
    {
        ...
        var keyParts = encryptedBlowfishKey.Take(SIZE_OF_ENCRYPTED_KEY).Chunk(SIZE_OF_RSA_KEY);
        ...
        foreach (var part in keyParts)
        {
            ...
            // Fill output key.
            Array.ConstrainedCopy(decrypted, 0, decryptedBlowfishKey, j, i + 1);
            j += i + 1;
        }
...
internal class BlowfishStream : Stream
{
    ...
    /// <summary>
    /// Encapsulated encrypted data stream.
    /// </summary>
    private readonly Stream stream;

    /// <summary>
    /// 56 byte long Blowfish key.
    /// </summary>
    ...
    public BlowfishStream(Stream stream, byte[] key)
    {
    ...
    private void DecryptBlock()
    {
        ...
        Decrypt(temp[0], temp[1], out temp[0], out temp[1]);
    ...
    private void KeyExpansion()
    {
        ...
        {
            Encrypt(left, right, out left, out right);
            subkeys[i] = left;
            subkeys[i + 1] = right;
        }
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                Encrypt(left, right, out left, out right);
                substitutions[i, j] = left;
                substitutions[i, j + 1] = right;
            }
        }
    }
}
